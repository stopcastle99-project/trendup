...
let i18n = {
  ko: { 
    title: "실시간 글로벌 트렌드", update: "최근 업데이트", summary: "AI 분석 리포트", news: "관련 뉴스", videos: "유튜브 소식", loading: "분석 중...", T: "트렌드 설정", L: "언어 설정", original: "원문",
    menu: { about: "TrendUp 소개", privacy: "개인정보처리방침", terms: "이용약관", contact: "문의하기", siteInfo: "사이트 정보" },
    pages: {
      about: { title: "TrendUp 소개", content: `<h2>TrendUp</h2><p>실시간 글로벌 트렌드 인텔리전스 플랫폼입니다.</p>` },
      privacy: { title: "개인정보 처리방침", content: `<h2>개인정보 처리방침 (v2.1.0)</h2><p>본 서비스는 개인정보를 소중히 다룹니다.</p>` },
      terms: { title: "이용약관", content: `<h2>이용약관</h2><p>참고용 데이터를 제공합니다.</p>` },
      contact: { title: "문의하기", content: `<p>help@trendup.ai</p>` }
    }
  },
  ja: { 
    ...
    pages: { about: { title: "TrendUpについて", content: `<h2>TrendUp</h2>` }, privacy: { title: "個人情報保護方針", content: `<p>v2.1.0</p>` }, terms: { title: "利用規約", content: `<h2>利用規約</h2>` }, contact: { title: "お問い合わせ", content: `<p>help@trendup.ai</p>` } }
  },
  en: { 
    ...
    pages: { about: { title: "About TrendUp", content: `<h2>TrendUp</h2>` }, privacy: { title: "Privacy Policy", content: `<p>v2.1.0</p>` }, terms: { title: "Terms of Service", content: `<h2>Terms of Service</h2>` }, contact: { title: "Contact Us", content: `<p>help@trendup.ai</p>` } }
  }
};
...
class TrendModal extends HTMLElement {
  ...
  show(trend, lang) {
    ...
    this.isVisible = true;
    const t = i18n[lang] || i18n.en;
    // DB에서 미리 생성된 AI 분석 리포트를 사용
    const analysis = trend.aiReport || (trend.snippets && trend.snippets.length > 0 ? trend.snippets.join(' ') : "분석 리포트를 생성할 수 없습니다.");
...
class App {
  ...
  refreshUIText() {
    try {
      ...
      if (footerText) footerText.textContent = `© 2026 TrendUp. All rights reserved. (v2.1.0)`;
    ...
    try {
      ...
      if (trendDoc.exists()) {
        ...
        const itemsMapped = dbData.items.map(item => {
          ...
          const aiReport = item.aiReports?.[this.currentLang] || ""; // DB에서 언어별 AI 분석본 로드
          
          return {
            ...item,
            displayTitle: originalTitle,
            translatedSubTitle: (translatedTitle !== originalTitle) ? translatedTitle : "",
            aiReport: aiReport // 화면 전달용
          };
        });
