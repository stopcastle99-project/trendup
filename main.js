...
class TrendModal extends HTMLElement {
  ...
  hide() { this.isVisible = false; const overlay = this.shadowRoot.querySelector('.overlay'); if (overlay) overlay.classList.remove('active'); }
  renderLoading() { this.shadowRoot.innerHTML = `<style>.overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.6); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; z-index: 9999; opacity: 1 !important; pointer-events: auto !important; transition: 0.3s; } .modal { background: var(--bg); width: 90%; max-width: 450px; border-radius: 24px; padding: 3rem 2rem; border: 1px solid var(--border); text-align: center; color: var(--text-muted); }</style><div class="overlay active"><div class="modal">Analyzing Trend...</div></div>`; this.shadowRoot.querySelector('.overlay').onclick = (e) => { if (e.target === e.currentTarget) this.hide(); }; }
