...
let i18n = {
  ...
  ja: { 
    ...
    cookie: "본사이트는 ユーザー体験向상의 위해 쿠키를 사용합니다.", accept: "확인",
    ...
    pages: {
      ...
      privacy: { title: "個人情報保護方針", content: `<h2>個人情報保護方針</h2><p>TrendUpは利用者の個人情報の保護를 최우선으로 합니다.</p><h3>1. 収集하는 情報</h3><p>当 서비스는 성함이나 메일 주소 등의 개인을 식별할 수 있는 정보를 수집하지 않습니다. 다만, 서비스 개선이나 통계 분석을 위해 쿠키나 액세스 로그가 자동으로 생성·수집될 수 있습니다. (v1.5.1)</p>` },
  ...
  en: { 
    ...
    pages: {
      ...
      privacy: { title: "Privacy Policy", content: `<h2>Privacy Policy</h2><p>We prioritize your privacy and comply with global data protection standards. (v1.5.1)</p>` },
...
class App {
  ...
  async init() {
    ...
    this.renderNavs();
    await this.update();
    this.backgroundSyncAll(); // Start Phase 13 Background Global Sync
  ...
  async backgroundSyncAll() {
    ...
    for (const c of countries) {
      if (c.code === this.currentCountry) continue; // Current already updated
      try {
        ...
        if (!dbData || (now - lastUpdated > this.service.refreshInterval)) {
          console.log(`Pre-emptively fetching trends for ${c.code}...`);
          ...
          await new Promise(r => setTimeout(resolve, 2000)); // Delay between countries
        }
