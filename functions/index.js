...
class TrendUpdater {
  ...
  getFallbackReport(title, lang, country) {
    ...
    if (lang === 'ja') return `${countryName}内で'${title}'가注目を集めています.`;
...
export const scheduledTrendUpdate = onSchedule({
  schedule: "every 30 minutes", // Increased to 30 mins to save quota
  secrets: ["GEMINI_API_KEY"],
  timeoutSeconds: 540
}, async (event) => {
  const updater = new TrendUpdater();
  await updater.updateAll();
  console.log("Efficient trend update cycle completed.");
});
