...
file_path = '/Users/stopcastle/trendup/main.js'
...
if idx > -1:
    new_content = new_i18n + "\\n\\n" + content[idx:]

    import re
    # Replace the update target for last-updated to properly translate when changing lang
    pattern = r"const cookieBtn = document\.getElementById\('accept-cookies'\);\s+if \(cookieBtn && t\.pages\.cookie\) cookieBtn\.textContent = t\.pages\.cookie\.btn;\s+this\.updateGeminiUsage\(\);"
    
    replacement = r'''const cookieBtn = document.getElementById('accept-cookies');
    ...
    print("Could not find firebaseConfig anchor.")
